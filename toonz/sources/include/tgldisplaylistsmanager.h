...
/*!
  ...
  a display lists space without having to access any actual associated OpenGL context. 
  This is equivalent to making a hidden OpenGL context (the display lists proxy) the \a current one.
...
public:
  virtual ~TGLDisplayListsProxy() = default;

  // Prevent copying and moving
  TGLDisplayListsProxy(const TGLDisplayListsProxy&) = delete;
  TGLDisplayListsProxy& operator=(const TGLDisplayListsProxy&) = delete;
  TGLDisplayListsProxy(TGLDisplayListsProxy&&) = delete;
  TGLDisplayListsProxy& operator=(TGLDisplayListsProxy&&) = delete;
...
public:
  explicit TGLDisplayListsProxyT(Context *proxy) : m_proxy(proxy) {}
  ~TGLDisplayListsProxyT() override { delete m_proxy; }

  // Prevent copying and moving
  TGLDisplayListsProxyT(const TGLDisplayListsProxyT&) = delete;
  TGLDisplayListsProxyT& operator=(const TGLDisplayListsProxyT&) = delete;
  TGLDisplayListsProxyT(TGLDisplayListsProxyT&&) = delete;
  TGLDisplayListsProxyT& operator=(TGLDisplayListsProxyT&&) = delete;
...
public:
  ...
  // Store the specified proxy and return its associated display lists id.
  // Takes ownership of the proxy pointer.
  int storeProxy(TGLDisplayListsProxy *proxy);

  // Attaches the specified context to a display lists space
  void attachContext(int dlSpaceId, TGlContext context);

  // Releases a context reference to its display lists space
  void releaseContext(TGlContext context);

  // Returns the display lists space id of a known context, or -1 if not found
  int displayListsSpaceId(TGlContext context);

  // Returns the display lists space proxy associated to the specified id.
...
private:
  ...
  TGLDisplayListsManager() = default;
  ~TGLDisplayListsManager() = default;

  // Prevent copying and moving
  TGLDisplayListsManager(const TGLDisplayListsManager&) = delete;
  TGLDisplayListsManager& operator=(const TGLDisplayListsManager&) = delete;
  TGLDisplayListsManager(TGLDisplayListsManager&&) = delete;
  TGLDisplayListsManager& operator=(TGLDisplayListsManager&&) = delete;
};
