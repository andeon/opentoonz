 ...
 *----------------------------------------------------------------------------*/
 ...
 #include <stdlib.h>
 #include <string.h>
 #include <stdint.h>  // For uint64_t, int64_t, etc.
 ...
 // Use standard types from <stdint.h>, avoid redefining
 typedef uint64_t ulong;  // Changed to 64-bit for consistency
 typedef uint16_t ushort; // Kept as 16-bit where appropriate
 typedef uint32_t uint;   // Define uint explicitly for MSVC
 
 // Remove redundant and conflicting typedefs
 // #ifndef MACOSX
 ...
 // Memory management variables (use uint for sizes)
 static long Prealloc[PREALLOC_LONGS];
 static void *Free_List[FREE_LISTS] = {0}; // Explicitly init to 0
 static char *Avail_Base = (char *)Prealloc;
 static uint Avail_Size = PREALLOC_SIZE;  // Use uint (uint32_t)
 ...
 TREE *avl__tree(int treetype, uint keyoffs, int (*usrcmp)(void *, void *)) {
   ...
   tree->usrcmp  = usrcmp;
   tree->nodes   = 0;  // Already int64_t in avl.h
   tree->path    = NIL;
 ...
 int avl_insert(TREE *tree, void *data) {
   ...
   case STR_KEY:
     ...
     break;
   case LNG_KEY:
     node->key.val = *(int64_t *)((char *)data + tree->keyoffs);
     break;
   case INT_KEY:
   ...
   case SHT_KEY:
     ...
     break;
   case ULN_KEY:
     node->key.val = CORRECT(*(uint32_t *)((char *)data + tree->keyoffs));
     break;
   case UIN_KEY:
     node->key.val = CORRECT(*(uint32_t *)((char *)data + tree->keyoffs));
     break;
   case USH_KEY:
     node->key.val = *(uint16_t *)((char *)data + tree->keyoffs);
     break;
   case CHR_KEY:
 ...
 void *avl__locate(TREE *tree, intptr_t keyval) {
   NODE *node;
   NODE *save;
 ...
 void *avl__locate_ge(TREE *tree, intptr_t keyval) {
   NODE *node;
   NODE *save;
   int (*usrcmp)(void *, void *);
   int cmp;
 ...
 void *avl__locate_gt(TREE *tree, intptr_t keyval) {
   NODE *node;
   NODE *save;
   int (*usrcmp)(void *, void *);
   int cmp;
 ...
 void *avl__locate_le(TREE *tree, intptr_t keyval) {
   NODE *node;
   NODE *save;
   int (*usrcmp)(void *, void *);
   int cmp;
 ...
 void *avl__locate_lt(TREE *tree, intptr_t keyval) {
   NODE *node;
   NODE *save;
   int (*usrcmp)(void *, void *);
   int cmp;
 ...
 static NODE *remove_val(NODE **rootaddr, int64_t keyval, int dup) {
   NODE *root = *rootaddr;
   NODE *node;
 ...
 void *avl__remove(TREE *tree, intptr_t keyval) {
   NODE *node;
   void *data;
   ...
   if (tree->root) {
         ...
         case STR_NODUP:
             ...
             break;
         case COR_NODUP:
             keyval = CORRECT(keyval); // keyval is intptr_t, CORRECT adjusts
             node = remove_val(&tree->root, (int64_t)keyval, NODUP);
             break;
         case VAL_NODUP:
             node = remove_val(&tree->root, (int64_t)keyval, NODUP);
             break;
         ...
         case COR_DUP:
             keyval = CORRECT(keyval);
             node = remove_val(&tree->root, (int64_t)keyval, DUP);
             break;
         case VAL_DUP:
             node = remove_val(&tree->root, (int64_t)keyval, DUP);
             break;
 ...
 void *avl__start(TREE *tree, intptr_t keyval, int back) {
   PATH *path;
   char *pathright;
   NODE **pathnode;
   NODE *node;
 ...
 static int *Lev;
 static int *Pos;
 static int Nod;    // Changed to int to match avl.h
 static int Max_Lev; // Changed to int to match avl.h
 ...
 static int check_subtree(NODE *root, int lev) {
   int leftlev  = lev;
   int rightlev = lev;
 
   if (root->left) leftlev = check_subtree(root->left, lev + 1);
   if (leftlev == ERROR) return ERROR;
   if (root->right) rightlev = check_subtree(root->right, lev + 1);
   if (rightlev == ERROR) return ERROR;
 
   switch (root->bal) {
   case LEFT:
     if (leftlev <= rightlev) Check_Bal = FALSE;
     break;
   case BAL:
     if (leftlev != rightlev) Check_Bal = FALSE;
     break;
   ...
   return leftlev > rightlev ? leftlev : rightlev;
 }
 
 /*---------------------------------------------------------------------------*/
 
 int avl_check(TREE *tree) {
   Check_Bal = TRUE;
   if (tree->root) {
 ...
 int64_t avl_count(TREE *tree) { return tree->nodes; }
 
 /*---------------------------------------------------------------------------*/
 
 static int Avl_Size;
 
 static void count_subtree(NODE *root) {
   Avl_Size++;
   if (root->left) count_subtree(root->left);
   if (root->right) count_subtree(root->right);
 }
 
 int avl_size(TREE *tree) {
   Avl_Size = 0;
   if (tree->root) count_subtree(tree->root);
   return Avl_Size;
 }
 ...
 static void print_subtree(NODE *root, int lev, TREE *tree) { // Added TREE *tree parameter
   int i;
 
   if (root->left) print_subtree(root->left, lev + 1, tree);
   for (i = 0; i < lev; i++) fprintf(Out, "  ");
   switch (KEYTYPE(tree->keyinfo)) { // Use tree->keyinfo
   case LNG_KEY:
     fprintf(Out, "%" PRId64, root->key.val);  // int64_t
     break;
   case INT_KEY:
     fprintf(Out, "%d", (int)root->key.val);
     break;
   case SHT_KEY:
     fprintf(Out, "%d", (short)root->key.val);
     break;
   case ULN_KEY:
     fprintf(Out, "%" PRIu64, (ulong)(root->key.val - MINLONG));  // uint64_t
     break;
   case UIN_KEY:
     fprintf(Out, "%u", (uint32_t)(root->key.val - MINLONG));
     break;
   case USH_KEY:
     fprintf(Out, "%u", (uint16_t)root->key.val);
     break;
   case CHR_KEY:
     fprintf(Out, "%c", (char)root->key.val);
     break;
   case STR_KEY:
     fprintf(Out, "%s", (char *)root->key.ptr);
     break;
   case PTR_KEY:
     fprintf(Out, "%p", root->key.ptr);
     break;
   case MBR_KEY:
     fprintf(Out, "%p", root->key.ptr);
     break;
   ...
   fprintf(Out, "\n");
   if (root->right) print_subtree(root->right, lev + 1, tree);
 }
 
 /*---------------------------------------------------------------------------*/
 
 void avl_print(TREE *tree, FILE *out) {
   Out = out;
   if (tree->root) print_subtree(tree->root, 0, tree); // Pass tree parameter
 }
 
 /*===========================================================================*/
 
 static const char *Hex = "0123456789ABCDEF"; // Added const
 
 static void parse_color(char *color, char *buf) {
   char *p;
 ...
 int avl_color(char *color) {
   char buf[16];
   int r, g, b, a;
 
   parse_color(color, buf);
   a = (hex(buf[0]) << 4) + hex(buf[1]);
   r = (hex(buf[2]) << 4) + hex(buf[3]);
   g = (hex(buf[4]) << 4) + hex(buf[5]);
   b = (hex(buf[6]) << 4) + hex(buf[7]);
   return (a << 24) + (r << 16) + (g << 8) + b;
 }
 
 /*---------------------------------------------------------------------------*/
 
 int avl_red(int argb) { return (argb >> 16) & 0xFF; }
 
 int avl_green(int argb) { return (argb >> 8) & 0xFF; }
 
 int avl_blue(int argb) { return argb & 0xFF; }
 
 int avl_alpha(int argb) { return (argb >> 24) & 0xFF; }
 ...
 TREE *cdb_decode_all(char *names_in_plt_file, TCM_INFO tcm) {
   ...
   char tmp[100];
   int64_t i; // Use int64_t for large counts
   size_t p;  // Use size_t for parameter indices
   CDB_TREE_ITEM *item;
   CDB_EFFECT *effect;
 
   tree = avl__tree(AVL_NODUP_INT, offsetof(CDB_TREE_ITEM, index), NULL);
 
   item   = NIL;
   endstr = strchr(names_in_plt_file, '\0');
   c1     = names_in_plt_file;
   c2     = NIL;
   for (i = 0; i < (int64_t)(tcm.n_colors + tcm.n_pencils); i++) { // Cast to int64_t
     CDB_TCALLOC(item, 1)
     item->index = (int)i; // Assuming index is int; adjust if int64_t needed
     ...
     if (*c2 == '\t') {
       if (c2 - c1 == 1 && *c1 == '*')
         ...
         item->group = strnsave(c1, (size_t)(c2 - c1));
       c1            = c2 + 1;
       c2            = strchr(c1, '\t');
       if (!c2) c2   = endcol;
     } else {
     ...
     while (*c1 == '<' && c1[1] == '!') {
       ...
       if (*c1 == 'A') {
         c1++;
         item->accelerator = strnsave(c1, (size_t)(ct - c1));
       }
       c1 = ct + 2;
     }
     if (c2 - c1 == 1 && *c1 == '*')
       ...
       item->name = strnsave(c1, (size_t)(c2 - c1));
     ...
     if (!*c1) {
       ...
       item->effects = avl__tree(AVL_NODUP_STR, offsetof(CDB_EFFECT, name), NULL);
       for (;;) {
         ct = strstr(c1, " ! ");
         if (ct && ct < endcol) {}
         ct = strstr(c1, "#! ");
         ...
         effect->name = strnsave(c1, (size_t)(c2 - c1));
         ...
         tmp[c2 - c1]       = '\0';
         effect->num_params = (int)strtoll(tmp, NULL, 10); // Replace _atoi64 with strtoll
         if (effect->num_params) CDB_TCALLOC(effect->param, effect->num_params)
         c1 = c2 + 1;
         for (p = 0; p < effect->num_params; p++) {
           c2 = strchr(c1, '#');
           if (!c2) goto error;
           effect->param[p].name = strnsave(c1, (size_t)(c2 - c1));
           c1                    = c2 + 1;
           c2 = (p == effect->num_params - 1) ? endcol : strstr(c1, " : ");
           if (!c2) goto error;
           effect->param[p].value = strnsave(c1, (size_t)(c2 - c1));
